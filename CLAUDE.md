...
- **`agents.lua`** — `Ctrl+Shift+G` toggle for Claude Code background agents: first press enumerates them via `claude agents --json` and opens each in its own tab running `claude attach <id>`; second press detaches them all by killing the attach panes via `wezterm cli kill-pane` (agents keep running); `opened_panes` is module-level mutable state; injects OSC 7 at spawn so splits from agent tabs inherit the agent's project dir (the Claude TUI emits no OSC 7 and the attach client's process cwd is `$HOME`, so cwd divining fails)
