...
- Hook events: `PreToolUse` → running, `Notification` (permission_prompt, elicitation_dialog only) → asking, `Stop` → idle, `SessionEnd` → clear (no argument); configured in `~/.claude/settings.json`
